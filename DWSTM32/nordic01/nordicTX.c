...
static void nordic_delay ( unsigned int n )
{
    unsigned int ra;
...
{
    spi16(0x2770); //Clear any interrupts
    spi8(0xE1); //Clear TX Fifo
    spi_transfer_four(0xA0,x);
    nordic_ce(1);
    nordic_delay(24);
    nordic_ce(0);
}
...
int notmain ( void )
{
    ...
    spi16(0x2078); //crc 1 byte, power down, ptx
    spi16(0x2100); //disable auto-acknowledge
    spi16(0x2770); //clear what you can
    spi16(0x207A); //power up

    hexstring(spi16(0));

    send_packet(0x12345678);

    while(1)
    {
        ra=uart_recv();
        send_packet(ra|(ra<<16));
    }
