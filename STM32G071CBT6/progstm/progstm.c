...
//-----------------------------------------------------------------------------
int write_mem_64 ( unsigned int add, unsigned int *data )
{
    unsigned int ra,rb,rc;

    printf("write_mem_64(0x%08X)\n",add);
    sdata[0]=0x31;
    sdata[1]=sdata[0]^0xFF;
    ser_senddata(sdata,2);
    while(1)
    ...
    for(rb=0;rb<16;rb++)
    {
        sdata[rc++]=(data[rb]>> 0)&0xFF;
        sdata[rc++]=(data[rb]>> 8)&0xFF;
        sdata[rc++]=(data[rb]>>16)&0xFF;
        sdata[rc++]=(data[rb]>>24)&0xFF;
    }
    xor_data(sdata,rc++);
    ser_senddata(sdata,rc);
    while(1)
    {
        rb=ser_copystring(rdata);
        if(rb)
...
//-----------------------------------------------------------------------------
int extended_erase_page ( unsigned int page )
{
    unsigned int ra,rb,rc;

    printf("extended_erase_page(%u)\n",page);
    sdata[0]=0x44;
    sdata[1]=sdata[0]^0xFF;
    ser_senddata(sdata,2);
    while(1)
...
int main ( int argc, char *argv[] )
{
    ...
    for(ra=0;ra<binlen;ra+=16)
    {
        if(write_mem_64(0x08000000+(ra<<2),&bindata[ra])) return(1);
    }
...
if(1)
{
    ...
    {
        if(read_mem_32(0x08000000+(ra<<2),&rb)) return(1);
        if(rb!=bindata[ra])
        {
            printf("VERIFY ERROR 0x%08X 0x%08X 0x%08X\n",0x08000000+(ra<<2),rb,bindata[ra]);
            errors++;
            if(errors>20) break;
        }
    }
